...
int main(int argc, char* argv[]) {
	...
	std::ofstream fout;
	fout.open(Form("/home/fas/caines/ly247/Scratch/pythiadata/Xsec4pythia8215_pp200hard_MPIoff_PionDecayOff_seed%s.csv",seed.data()));
	...
	for (int iBin = 0; iBin < nBin; ++iBin) {
		...
		// MPI Off
		pythia.readString("PartonLevel:MPI = off"); // switch off MPI
		...
		TFile* outFile = new TFile(Form("~/Scratch/pythiadata/pythia8215_pp200hard_MPIoff_pT%d-%d_PionDecayOff_seed%s.root",int(pTlimitLow[iBin]),int(pTlimitLow[iBin+1]),seed.data()), "RECREATE");
